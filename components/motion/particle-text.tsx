...
const waitForFonts = async (font: string): Promise<void> => {
  ...
  try {
    ...
    // font failed to load — sample with whatever's available
  }

  await document.fonts.ready
}

// Vendored from React Bits (https://reactbits.dev). Fully self-contained:
// its own canvas, its own ResizeObserver, listeners only on its own
// container/canvas element — no `window` listeners, no globals, nothing
// that could interact with the scroll-driven mascot dock elsewhere on the
// page. Logic is unchanged from the vendored source; only converted to this
// project's no-semicolon/double-quote style.
const ParticleText = ({
  text = "React Bits",
  particleSize = 2,
  density = 4,
  ...
  return (
    <div
      ref={containerRef}
      // min-h-[130px] not 240px (the vendored default): this now renders
      // inside a compact rounded card, not a full hero section, and a
      // className override here wouldn't reliably win a Tailwind
      // specificity tie against this same base string's own class — this
      // has to be the actual default, not something fought from outside.
      className={`relative block h-full min-h-[130px] w-full touch-none overflow-hidden ${className}`}
      style={style}
      aria-label={text}
    >
      <canvas
